...
def CPD(img1_o,img2_o,Multimodal=False,Multistart=False,Sym=False,Param=False,border=0, artifacts = True):
    ...
    if Multimodal:
        ...
        img2_crop = ski.util.crop(img2,((width[0],width[1]),
                                        (width[2],width[3])),copy=True)
        scale = np.max(img1.shape[0:2]) / np.max(img2_crop.shape[0:2])
        img2 = cv2.resize(img2_crop,(0,0),fx=scale,fy=scale)
    ...
    if Multistart:
       ...
       for i, s in enumerate(starts, 1) :
           ...
           s_new = cv2.warpAffine(s, T, dsize = (img1.shape[1],img1.shape[0]), borderValue = background)
           if Multimodal:
